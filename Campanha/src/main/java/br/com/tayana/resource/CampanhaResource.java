...
import br.com.tayana.command.model.FindCampanhasModificadasPorDataCmd;
...
public class CampanhaResource {
	...
	public List<CampanhaResponse> findCampanhasAtivasPorTime(Integer id) {
		...
		if (cmd.getResponses() == null) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Não há campanhas para este time.");
		}

		return cmd.getResponses();
	}

	@GetMapping("/modificadas/{data}")
	public List<CampanhaResponse> findCampanhasModificadasAposData(LocalDate data) {
		FindCampanhasModificadasPorDataCmd cmd = commandFactory.createInstance(FindCampanhasModificadasPorDataCmd.class);
		cmd.setDataModificacao(data);
		cmd.execute();
